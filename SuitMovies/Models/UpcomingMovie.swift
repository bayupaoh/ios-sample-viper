...
class UpcomingMovie : Object {
    ...
    public static func with(realm: Realm, json: JSON) -> UpcomingMovie? {
        let identifier = json["id"].int64Value
        if identifier == 0 {
            return nil
        }
        var obj = realm.object(ofType: UpcomingMovie.self, forPrimaryKey: identifier)
        if obj == nil {
            obj = UpcomingMovie()
            obj?.identifier = identifier
        } else {
            obj = UpcomingMovie(value: obj!)
        }
    ...
    public static func with(json: JSON) -> UpcomingMovie? {
